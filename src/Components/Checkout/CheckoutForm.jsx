...
const CheckoutForm = () => {
    ...
    const checkOutHandler = async (e) => {
        ...
        if (!userDetails.firstName || !userDetails.lastName || !userDetails.userEmail || !userDetails.phoneNumber || !userDetails.address || !userDetails.zipCode || !userDetails.city || !userDetails.userState) {
            ...
            try {
                const {data} = await axios.post(`${process.env.REACT_APP_BASE_URL}/api/order/addorder`, { amount:totalAmount, userId:userData._id, userDetails:JSON.stringify(userDetails), productDetails:JSON.stringify(cart)}, {
                headers: {
                    'Authorization': authToken
                }
            })
