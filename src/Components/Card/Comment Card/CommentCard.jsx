...
const CommentCard = ({ userReview, setReviews, reviews, fetchReviews }) => {
    ...
    const handleDeleteComment = async () => {
        try {
            const { data } = await axios.delete(`${process.env.REACT_APP_BASE_URL}/api/review/deletereview/${userReview._id}`, {
                headers: {
                    'Authorization': authToken
                }
            })
    ...
    const deleteCommentByAdmin = async () => {
        if (isAdmin) {
            try {
                const { data } = await axios.delete(`${process.env.REACT_APP_BASE_URL}/api/admin/review/${userReview._id}`,
                    {
                        headers: {
                            'Authorization': authToken
                        }
    ...
    const sendEditResponse = async () => {
        if (!editComment && !value) {
            ...
            try {
                if (authToken) {
                    const response = await axios.put(`${process.env.REACT_APP_BASE_URL}/api/review/editreview`,
                ...
                toast.error(error,"Something went wrong", { autoClose: 600, })
            }
        }
    }
    return (
