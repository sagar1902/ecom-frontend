...
const UpdateDetails = () => {
    ...
    const getUserData = async () => {
        try {
            const { data } = await axios.get(`${process.env.REACT_APP_BASE_URL}/api/auth/getuser`, {
                headers: {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!userDetails.email || !userDetails.firstName || !userDetails.phoneNumber || !userDetails.lastName || !userDetails.address || !userDetails.city || !userDetails.userState || !userDetails.zipCode) {
                ...
                toast.error("Please enter state,more than 3 characters", { autoClose: 500, theme: 'colored' })
            }
            else if (userDetails.city.length <= 3) {
                toast.error("Please enter city,more than 3 characters", { autoClose: 500, theme: 'colored' })
            }
            else {
                const { data } = await axios.put(`${process.env.REACT_APP_BASE_URL}/api/auth/updateuser`, {
    ...
    const handleResetPassword = async (e) => {
        ...
        try {
            if (!password.currentPassword || !password.newPassword) {
                ...
                const { data } = await axios.post(`${process.env.REACT_APP_BASE_URL}/api/password/reset/password`, {
                    id: userData._id,
    ...
    const deleteAccount = async () => {
        try {
            const deleteUser = await axios.delete(`${process.env.REACT_APP_BASE_URL}/api/auth/delete/user/${userData._id}`, {
                headers: {
                    'Authorization': authToken
                }
            });
