...
const ProductDetail = () => {
    ...
    const addToCart = async (product) => {
        if (setProceed) {
            try {
                const { data } = await axios.post(`${process.env.REACT_APP_BASE_URL}/api/cart/addtocart`, { _id: product._id, quantity: productQuantity }, {
                    headers: {
    ...
    const shareProduct = (product) => {
        ...
        const data = {
            text: product.name,
            title: "e-shopit",
            url: `http://localhost:5000/Detail/type/${cat}/${id}`
        }
    ...
    const getSimilarProducts = async () => {
        const { data } = await axios.post(`${process.env.REACT_APP_BASE_URL}/api/product/fetchproduct/type`, { userType: cat })
