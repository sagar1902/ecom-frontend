...
const Cart = () => {
    ...
    useEffect(() => {
        if (setProceed) {
            cart.products && setTotal(cart.products?.reduce((acc, curr) => {return (acc+ ((curr.productId?.price * curr.quantity)))}, shippingCost))
        }
    ...
    const getCart = async () => {
        if (setProceed) {
            const { data } = await axios.get(`${process.env.REACT_APP_BASE_URL}/api/cart/fetchcart`,
                {
                    headers: {
    ...
    const removeFromCart = async (product) => {
        if (setProceed) {
            try {
                const response = await axios.delete(`${process.env.REACT_APP_BASE_URL}/api/cart/deletefromcart/${product._id}`, {
                    headers: {
                        'Authorization': authToken
                    }
                })
                ...
                <Container sx={{ display: 'flex', flexDirection: "column", mb: 10 }}>
                    ...
                    </Box>
                    {
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                            <OrderSummary proceedToCheckout={proceedToCheckout} total={total} shippingCoast={shippingCost} />
                        </Box>
                    }
