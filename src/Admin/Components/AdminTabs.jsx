...
export default function BasicTabs({ user, setUser, getUser }) {
    ...
    const totalSales = paymentData.reduce((acc, curr) => (acc + curr.totalAmount), 0);
    // const totalSales = 0
        ...
        <Box sx={{ width: '100%' }}>
            <Grid container spacing={2} direction={isSmallScreen ? 'column' : 'row'} padding={1}>
                <Grid item xs={12} sm={6} md={6} lg={3}>
                    <Widget numbers={totalSales} heading='Sales' color='#9932CC' icon={<TbReportMoney />} />
                </Grid>
                <Grid item xs={12} sm={6} md={6} lg={3}>
                    <Widget numbers={products?products.length:0} heading='Products' color='#FFC300' icon={<AiOutlineShoppingCart />} />
                </Grid>
                <Grid item xs={12} sm={6} md={6} lg={3}>
                    <Widget numbers={user?user.length:0} heading='Users' color='#FF69B4' icon={<CgProfile />} />
                </Grid>
                <Grid item xs={12} sm={6} md={6} lg={3}>
                    <Widget numbers={paymentData?paymentData.length:0} heading='Orders' color='#1f77b4  ' icon={<FaShippingFast />} />
                </Grid>
            </Grid>
