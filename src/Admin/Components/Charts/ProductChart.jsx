...
// console.log(cart)
    const productData = [
        ...
        {
            name: "Shoes",
            Quantity: products.filter(prod => prod.type === "shoes").length
        },
        {
            name: "Electronics",
        ...
        {
            name: "Books",
            Quantity: products.filter(prod => prod.type === "books").length
        },
        // {
        //     name: "Jewelry",
    ...
    //           console.log(acc1, curr1);
    //           return (acc1 + (curr1.productId?.type === cat ? curr1.quantity : 0));
    //         }, 0) || 0)
    //       ), 0);
          
    //     // cart.reduce((acc, curr) => (acc + curr.products?.reduce((acc1, curr1)=>{console.log(acc1, curr1);return (acc1+(curr1.productId.type===cat?curr1.qunatity:0))},0)), 0)
    // }
    function cartutil(cat) {
        let total = 0;
        
        for (const curr of cart) {
          if (curr.products) {
            for (const curr1 of curr.products) {
              if (curr1.productId && curr1.productId.type === cat) {
                total += curr1.quantity;
              }
            }
          }
        }
    ...
    const cartData = [
        {
            name: "Cloths",
            "Quantity in cart": cartutil('cloths')//cart[0]?.products.filter(prod => prod.productId && prod.productId.type === "cloths").length
        },
        {
            name: "Shoes",
            "Quantity in cart": cartutil('shoes')//cart[0]?.products.filter(prod => prod.productId && prod.productId.type === "shoe").length
        },
        {
            name: "Electronics",
            "Quantity in cart": cartutil('electronics')//cart[0]?.products.filter(prod => prod.productId && prod.productId.type === "electronics").length
        },
        {
            name: "Books",
            "Quantity in cart": cartutil('books')//cart[0]?.products.filter(prod => prod.productId && prod.productId.type === "book").length
        },
        // {
        //     name: "Jewelry",
        //     "Quantity in cart": cart.filter(prod => prod.productId && prod.productId.type === "jewelry").length
        // },
    ];
