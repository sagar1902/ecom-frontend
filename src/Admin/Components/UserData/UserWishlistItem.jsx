...
const UserWishlistItem = ({ authToken, id, commonGetRequest }) => {
    ...
    useEffect(() => {
        commonGetRequest(`${process.env.REACT_APP_BASE_URL}/api/admin/getwishlist`, id, setUserWishlist);
    }, [])
    const removeCartItemByAdmin = async (product) => {
        try {
            const { data } = await axios.delete(`${process.env.REACT_APP_BASE_URL}/api/admin/userwishlist/${product._id}`, {
                headers: {
